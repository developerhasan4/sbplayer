...
package
{
	...
	public class TimeSliderTooltipPlugin extends Sprite implements IPlugin 
	{
		...
		private function traceConfig(obj:Object):void
		{
			// TODO Auto Generated method stub
			for(var pam:* in obj){
				Logger.log(pam.toString()+": "+obj[pam].toString(),'TST');
			}
		}
		...
		private function createTooltip():void
		{
			if ( null == _tooltip )
			{
				...
				_bg.x = -_bg.width / 2;
				//_bg.y = -_bg.height;
			//	_tooltip.height = _bg.height;
				//_tooltip.height = _bg.height+10;
...
//				_txt.addEventListener(TextEvent.LINK,onLink);
				...
				Logger.log(_tooltip.height,'TST');
			}
		}
		...
		private function setupTooltip():void
		{
			...
			if (null != _timeSlider)
			{
				...
				//_tooltip.buttonMode = true;
				
				_tooltip.addEventListener(MouseEvent.MOUSE_OVER,onTooltipOver,true);
				_tooltip.addEventListener(MouseEvent.MOUSE_OUT,onTooltipOut,true);
			}			
		}
		
		protected function onTooltipOut(event:MouseEvent):void
		{
			// TODO Auto-generated method st
			_tooltip.visible = false;
		}
		
		protected function onTooltipOver(event:MouseEvent):void
		{
			// TODO Auto-generated method stub
		...
		private function onMediaTimeHandler(event:MediaEvent):void 
		{
			...
			_txt.htmlText = '<a href="event:'+_duration+'">'+toTimeString(Math.round(percent * _duration))+'</a>';
			//_txt.htmlText = '<i>'+toTimeString(Math.round(percent * _duration))+'</i>';
			
		}
		
		private function onMouseHandler(event:MouseEvent):void 
		{
			Logger.log(event.type,'TST');
